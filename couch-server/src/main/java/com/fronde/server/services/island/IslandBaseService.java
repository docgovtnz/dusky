...
@Component
public abstract class IslandBaseService {
  ...
  public IslandEntity findById(String docId) {
    // using orElse instead of get so we return null if the entity is not found
    return repository.findById(docId).orElse(null);
  }

  public PagedResponse<IslandEntity> search(IslandCriteria criteria) {
    return repository.findByCriteria(criteria);
  }

  public Response<IslandEntity> save(IslandEntity entity) {
    ...
    Response<IslandEntity> response = new Response<>(entity);
  ...
  public void export(IslandCriteria criteria, HttpServletResponse response) {
    List<String> header = Arrays.asList("Name", "ID");
    List<String> props = Arrays.asList("name", "islandId");
    criteria.setPageSize(exportUtils.getMaxRows());
    criteria.setPageNumber(1);
    PagedResponse<IslandEntity> pr = this.search(criteria);
    exportUtils.export(response, pr, header, props, "island");
  }
