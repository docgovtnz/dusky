...
@Component
public abstract class LocationBaseService {
  ...
  public LocationEntity findById(String docId) {
    // using orElse instead of get so we return null if the entity is not found
    return repository.findById(docId).orElse(null);
  }

  public PagedResponse<LocationEntity> search(LocationCriteria criteria) {
    return repository.findByCriteria(criteria);
  }

  public Response<LocationEntity> save(LocationEntity entity) {
    ...
    Response<LocationEntity> response = new Response<>(entity);
  ...
  public void export(LocationCriteria criteria, HttpServletResponse response) {
    ...
    List<String> props = Arrays.asList(
        "locationName", "island", "locationType", "easting", "northing", "birdID",
        "active");
    criteria.setPageSize(exportUtils.getMaxRows());
    criteria.setPageNumber(1);
    PagedResponse<LocationEntity> pr = this.search(criteria);
