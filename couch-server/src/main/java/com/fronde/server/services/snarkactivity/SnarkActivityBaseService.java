...
@Component
public abstract class SnarkActivityBaseService {
  ...
  public SnarkActivityEntity findById(String docId) {
    // using orElse instead of get so we return null if the entity is not found
    return repository.findById(docId).orElse(null);
  }

  public PagedResponse<SnarkActivityEntity> search(SnarkActivityCriteria criteria) {
    return repository.findByCriteria(criteria);
  }

  public Response<SnarkActivityEntity> save(SnarkActivityEntity entity) {
    ...
    Response<SnarkActivityEntity> response = new Response<>(entity);
  ...
  public void export(SnarkActivityCriteria criteria, HttpServletResponse response) {
    List<String> header = Arrays.asList(
        ...
        "Boom", "Ching", "Skraak", "Mating Sign", "Birds");
    List<String> props = Arrays.asList(
        "activityType", "locationID", "date", "trackActivity", "sticks", "grubbing",
        "boom", "ching", "skraak", "matingSign", "birds");
    criteria.setPageSize(exportUtils.getMaxRows());
    criteria.setPageNumber(1);
    PagedResponse<SnarkActivityEntity> pr = this.search(criteria);
    exportUtils.export(response, pr, header, props, "snarkactivity");
  }
