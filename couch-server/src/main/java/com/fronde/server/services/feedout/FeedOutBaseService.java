...
@Component
public abstract class FeedOutBaseService {
  ...
  public FeedOutEntity findById(String docId) {
    // using orElse instead of get so we return null if the entity is not found
    return repository.findById(docId).orElse(null);
  }

  public PagedResponse<FeedOutEntity> search(FeedOutCriteria criteria) {
    return repository.findByCriteria(criteria);
  }

  public Response<FeedOutEntity> save(FeedOutEntity entity) {
    ...
    Response<FeedOutEntity> response = new Response<>(entity);
  ...
  public void export(FeedOutCriteria criteria, HttpServletResponse response) {
    List<String> header = Arrays.asList("Location", "Date out", "Date in");
    List<String> props = Arrays.asList("locationID", "dateOut", "dateIn");
    criteria.setPageSize(exportUtils.getMaxRows());
    criteria.setPageNumber(1);
    PagedResponse<FeedOutEntity> pr = this.search(criteria);
    exportUtils.export(response, pr, header, props, "feedout");
  }
