...
@Component
public abstract class NestObservationBaseService {
  ...
  public PagedResponse<NestObservationEntity> search(NestObservationCriteria criteria) {
  ...
  public Response<NestObservationEntity> save(NestObservationEntity entity) {
    ...
    Response<NestObservationEntity> response = new Response<>(entity);
  ...
  public void export(NestObservationCriteria criteria, HttpServletResponse response) {
    List<String> header = Arrays.asList(
        "Location Type", "Date / Time", "Mother", "Child 1", "Child 2", "Child 3",
        "Child 4", "Island", "Location");
    List<String> props = Arrays.asList(
        "locationType", "dateTime", "birdID", "child1BirdID", "child2BirdID",
        "child3BirdID", "child4BirdID", "island", "locationID");
    criteria.setPageSize(exportUtils.getMaxRows());
    criteria.setPageNumber(1);
    PagedResponse<NestObservationEntity> pr = this.search(criteria);
    exportUtils.export(response, pr, header, props, "nestobservation");
  }
