...
@Component({
  selector: 'app-medication-edit-form',
  templateUrl: './medication-edit-form.component.html',
})
export class MedicationEditFormComponent implements OnInit {
  @Input()
  myFormArray: FormArray;

  @Output()
  oneOrMoreMed: EventEmitter<boolean> = new EventEmitter<boolean>();
  ...
  addItem() {
    ...
    if (!this.myFormArray.length) {
      this.oneOrMoreMed.emit(true);
    }
    (this.myFormArray as any).addItem(new MedicationEntity());
  }

  removeItem(i: number) {
    if (this.myFormArray.length === 1) {
      this.oneOrMoreMed.emit(false);
    }
