...
@Component({
  selector: 'app-chemistryassay-edit-form',
  templateUrl: './chemistryassay-edit-form.component.html',
})
export class ChemistryAssayEditFormComponent implements OnInit {
  @Input()
  ...
  addItem() {
    ...
    const fg = (this.myFormArray as any).addItem(new ChemistryAssayEntity());
  ...
  addAutofill(autofill: SampleResultAutofill) {
    if (autofill) {
      autofill.tests.map((test) => {
        ...
        newItem.controls.chemistryAssay.setValue(test);
      });
    }
  }
}
